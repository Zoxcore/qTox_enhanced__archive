...
typedef struct
{
    ...
    int16_t *farend_resampled;
    uint32_t farend_resampled_samples;
    int16_t *nearendNoisy_resampled;
    uint32_t nearendNoisy_resampled_samples;
    int16_t *out_resampled;
    uint32_t out_resampled_samples;
} aecmob_t;
...
int32_t WebRtcAecmCustom_upsample_16000_to_48000_basic(int16_t* in, int16_t *out, int32_t sample_count)
{
    if (sample_count < 1)
    {
        return -1;
...
int32_t WebRtcAecmCustom_downsample_48000_to_16000_basic(int16_t* in, int16_t *out, int32_t sample_count)
{
    if (sample_count < 3)
    {
        return -1;
...
int32_t WebRtcAecm_Free(void *aecmInst)
{
    ...
    free(aecm->farend_resampled);
    free(aecm->nearendNoisy_resampled);
    free(aecm->out_resampled);
...
int32_t WebRtcAecm_InitCustom(void *aecmInst, int32_t sampFreq)
{
    int32_t ret;
    if (sampFreq == 48000)
    {
        ret = WebRtcAecm_Init(aecmInst, 16000);
        aecmob_t *aecm = aecmInst;
        aecm->sampFreq_real = sampFreq;
        aecm->farend_resampled = NULL;
        aecm->nearendNoisy_resampled = NULL;
        aecm->out_resampled = NULL;
        aecm->farend_resampled_samples = 0;
        aecm->nearendNoisy_resampled_samples = 0;
        aecm->out_resampled_samples = 0;
    }
    else
    {
...
int32_t WebRtcAecm_BufferFarendCustom(void *aecmInst, int16_t *farend,
                                int16_t nrOfSamples)
{
    aecmob_t *aecm = aecmInst;
    if (aecm->sampFreq_real == 48000)
    {
        if (aecm->farend_resampled_samples != nrOfSamples)
        {
            aecm->farend_resampled = calloc(1, sizeof(int16_t) * (nrOfSamples / 3));
            aecm->farend_resampled_samples = nrOfSamples;
        }
        WebRtcAecmCustom_downsample_48000_to_16000_basic(farend, aecm->farend_resampled, nrOfSamples);
        const int32_t res = WebRtcAecm_BufferFarend(aecmInst, aecm->farend_resampled, nrOfSamples / 3);
        return res;
    }
    else
    {
...
int32_t WebRtcAecm_ProcessCustom(void *aecmInst, int16_t *nearendNoisy,
                           ...
                           int16_t nrOfSamples, int16_t msInSndCardBuf)
{
    ...
    if (aecm->sampFreq_real == 48000)
    {
        if (aecm->nearendNoisy_resampled_samples != nrOfSamples)
        {
            aecm->nearendNoisy_resampled = calloc(1, sizeof(int16_t) * (nrOfSamples / 3));
            aecm->nearendNoisy_resampled_samples = nrOfSamples;
        }

        if (aecm->out_resampled_samples != nrOfSamples)
        {
            aecm->out_resampled = calloc(1, sizeof(int16_t) * (nrOfSamples / 3));
            aecm->out_resampled_samples = nrOfSamples;
        }

        WebRtcAecmCustom_downsample_48000_to_16000_basic(nearendNoisy, aecm->nearendNoisy_resampled, nrOfSamples);
        const int32_t res = WebRtcAecm_Process(
                aecmInst,
                aecm->nearendNoisy_resampled,
                NULL,
                aecm->out_resampled,
                nrOfSamples / 3,
                msInSndCardBuf
                );
        WebRtcAecmCustom_upsample_16000_to_48000_basic(aecm->out_resampled, out, nrOfSamples / 3);
        return res;
    }
    else
    {
